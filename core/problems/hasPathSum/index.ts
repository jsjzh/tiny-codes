...
export const code1 = (root: ITreeNode, targetSum: number): boolean => {
  if (!root) return false;
  if (targetSum === root.val && !root.left && !root.right) return true;

  return code1(root.left, targetSum - root.val) || code1(root.right, targetSum - root.val);
};

// export const code2 = (root: ITreeNode, targetSum: number): boolean => {};
